...
func TestParser(t *testing.T) {
	...
	t.Run("one shot", func(t *testing.T) {
		msg, format, err := ParseTextMessage(msgData)
		test.That(t, err, test.ShouldBeNil)
		msg.CachedRawData = nil
		test.That(t, format, test.ShouldEqual, common.NavLink2Instance)
		test.That(t, msg, test.ShouldResemble, expected)
	})

	t.Run("preset parser format", func(t *testing.T) {
		msg, err := ParseTextMessageWithFormat(msgData, common.NavLink2Instance.Name())
		test.That(t, err, test.ShouldBeNil)
		msg.CachedRawData = nil
		test.That(t, msg, test.ShouldResemble, expected)
	...
	t.Run("invalid format for data is ignored", func(t *testing.T) {
		_, err := ParseTextMessageWithFormat(msgData, common.GarminCSV1Instance.Name())
...
func TestDYParse(t *testing.T) {
	...
	msg, finished, err := parser.ProcessMessage("$PDGY,000000,0,0,2,28830,0,0")
	test.That(t, err, test.ShouldBeNil)
...
func TestSZParse(t *testing.T) {
	...
	msg, _, err := ParseTextMessage(msgData)
	test.That(t, err, test.ShouldBeNil)
